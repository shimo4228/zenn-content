...
> **Superseded in part (2026-08-23)**: 下表の channel 行と文体の値は、
> 2026-08-12 の「note = JA 正本（ですます）/ Substack = EN 翻訳」への逆転を反映していない。
> **チャンネル→文体の実値の正本は `.claude/rules/zenn-writing.md`「チャンネル表」**（ADR-0010）。
> 下表は当時の記録として残す。運用の根拠として引かないこと。
...
## Addendum (2026-08-02): 環境依存記事の Human Narrative / Agent Handoff

ローカル設定を扱う記事では、「コピペで動くコード」の要件と読者環境ごとの差が衝突する。絶対パス、既存設定、シンボリックリンク、認証状態が異なる以上、一般化した完成コマンドをそのまま実行させると、安全性か再現性のどちらかを失う。一方、技術詳細を削れば実用軸の「内容を薄くしない」に反する。

そのため、次の条件をすべて満たす記事では **Human Narrative / Agent Handoff の二層構成を既定**とする。

1. 読者のローカル環境・設定・既存ファイルを変更する
2. 環境差によって正しい手順が変わる
3. コーディングエージェントが読み取り専用の事前調査から個別計画を作れる

前半は人間向けナラティブとして、問題、既存手段が足りない理由、採用境界、トレードオフ、成功状態までを完結させる。後半は単独でエージェントへ渡せる実装契約とし、読み取り専用調査、変更対象・非対象、上書き禁止、復旧方法、実測の成功条件、未検証事項、人間承認点を要求する。エージェントは計画で停止し、明示承認まで実装しない。

これは全記事共通のテンプレートではない。安全で安定した単一コマンドや環境差のない例は、従来どおり人間向け how-to とする。また agent handoff は情報削減の口実にしない。人間の判断に必要な技術的根拠は前半に残し、環境固有の実装条件だけを後半へ移す。

検討した代替は、(a) 全手順を人間向けコマンドとして掲載する、(b) 記事全体をエージェント向け仕様にする、(c) 全 Zenn 記事へ二層構成を強制する、の 3 つ。(a) は環境差のある変更で危険、(b) は人間読者が設計を判断できない、(c) は単純な記事に不要な定型を持ち込むため採用しない。

正本は `zenn-practical-writing` の構成規約。採否は `zenn-editorial-judgment`、公開前の条件確認は `quality-gate` が担当する。最初の基準稿は `articles/claude-code-claudemd-excludes.md`。
