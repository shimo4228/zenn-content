...
5. **`title-eval` は維持する**。タイトルは唯一レビュアーが存在しない層で、判定器を置く根拠が残る。
   ...
   > **2026-08-23 追記（同日中に supersede）**: この Decision は撤回した。`title-eval` を
   > `title-reviewer` agent へ変換し、判定器は 0 本になった。理由は Decision 5 が置いた根拠
   > （レビュアー不在の層）が、判定器を置いてよい根拠として不十分だったこと — `title-eval` の
   > verdict は `Adopt / Refine / Keep current` の推奨で、最終選択は常に著者にあった。決着が
   > 著者の中にある層に判定器を名乗らせると、判定器の性能を測っているつもりの状態が続く
   > （本 ADR Context 2 でテーマ判定器について書いたのと同じ形）。findings だけを返す
   > `title-reviewer` に置き換え、採否は著者が持つ。
