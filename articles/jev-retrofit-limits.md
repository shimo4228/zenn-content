...
![1行足しても、スキルを選ぶ役はClaude Codeが持ったまま（59本の説明文を切らずに読んで選ぶ）で、Jevの判定（0.7〜1.6秒、adr-writerなどを1行で提案）は横を並走している](/images/jev-retrofit-limits-hero.png)

作ったものの形は、この1枚に収まります。Jevが渡せるのは1行のメモで、スキルを選ぶハンドルはClaude Codeが握ったままでした。
...
![cookbookの実験は選び手がclaude-haiku-4-5で見えるのは60字の索引、Claude Codeは選び手がClaude Fable 5.1でdescriptionは1,536字まで全文。同じ1行のメモでも渡す相手が違う](/images/jev-retrofit-limits-conditions.png)

2つの条件を並べると、違うのはメモを受け取る選び手の強さと、選び手に見えていた文字数です。
...
![スキル一覧に手を入れる3つの道。道1は1行足す（私はここで止めた）、道2は一覧を書き換える（前日にjev-skill-suggestionが実装済み）、道3は公式設定skillOverridesで一覧を軽くする（JevもModsも要らない）](/images/jev-retrofit-limits-three-roads.png)

3つの道を並べると、私が止めた道1の隣で、道2は前日に作られていて、道3の用途は公式の設定で足りていました。
...
![選び手の強さと、Jevの届き方の2軸。弱い選び手への助言は効いた（cookbook）、強い選び手への助言は効く見込みが小さい（Claude Code + 1行）、強い選び手の選ぶ役そのものに届くなら変わりうる（Mods / pi-jev）](/images/jev-retrofit-limits-who-picks.png)

選び手の強さと届き方の2軸で置くと、私の実装は左下の、効く見込みが小さい枠にあります。
