...
![1行足しても、スキルを選ぶ役はClaude Codeが持ったままで、Jevの判定は横を並走している](/images/jev-retrofit-limits-hero.png)
...
![Jevの判定をClaude Codeに届ける3つの道。道1は1行足す（私はここで止めた）、道2は一覧を書き換える（前日にjev-skill-suggestionが実装済み）、道3は公式設定skillOverridesで一覧を軽くする](/images/jev-retrofit-limits-three-roads.png)

3つの道を並べると、私が止めた道1の先で、道2は前日に作られていて、道3の用途は公式の設定で足りていました。
