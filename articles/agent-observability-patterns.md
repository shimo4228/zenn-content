...
私の運用でこれが解決したのは、「なんとなく同じ話ばかり溜まっている気がする」という体感の数値化です。テキストコーパスを蓄積するエージェントに、「消費される区分ごとの供給量」と「プール全体のコサイン類似度の分布」を報告するモジュールを足しました。リバランスすべきかどうかを、体感ではなく読みで判断できるエコーチェンバー検出器です。
