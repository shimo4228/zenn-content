...
## OTel を入れるかどうかは「誰がテレメトリを読むか」で決まります

エージェントに OTel を入れるか、見送るか。この記事ではその判断基準を 1 つに絞ります。「そのテレメトリを誰が読むのか」です。実際、OTel のトレースと筆者の監査ログは、同じイベントを記録していても**読む人（消費者）が違います**。たとえば 1 回の LLM 呼び出しなら、トレース側で見たいのはモデル名・トークン数・所要時間・エラーの有無まで。監査ログ側に要るのは、プロンプト本文そのもの・呼び出し元・再現に使う識別子です。
