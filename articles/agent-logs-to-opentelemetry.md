...
> **この記事でわかること**: 自前ログを OpenTelemetry につなぐと何が見えるのか。既存の JSONL ログをコードを変えずにトレースに変換して Jaeger で確かめ、OTel から借りる価値のある要素と見送ってよい要素を整理します。
...
| semantic conventions パッケージからの属性名 import | ❌ 見送り | Development 段階で import パスがまだ不安定なため、属性名は文字列定数で自前管理 |
