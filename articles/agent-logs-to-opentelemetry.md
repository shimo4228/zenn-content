...
- **LLM 呼び出しをどの属性名で記録するかの標準（GenAI semantic conventions）が、まだ Development ステータス。** 仕様が安定する前の段階なので、どこまで乗っていいか判断できない
...
なお「一部だけ使うのは規格のつまみ食いでは？」と思うかもしれません。OTLP は公開プロトコル、semantic conventions は属性名の取り決めなので、SDK を経由せずにデータを投入するのは規格が想定している使い方です。
...
面白いのは**写らなかった側**です。`caller`（障害調査でログを処理段ごとに集計するためのキー）や `prompt_sha256`（本文を保存せずに同一プロンプトを突き合わせるためのハッシュ）には標準の対応がありません。標準が表現できない部分は、そのまま「画面で調べるときには要らないが、障害のオフライン再現には要る情報」のリストになっていました。
...
見送った要素は、OTel のその部分に価値がないという意味ではありません。runtime 計装だけが与えるものは、正確な因果構造・サービス間のコンテキスト伝播・リアルタイム性・メトリクスの 4 つです。このうち後ろの 3 つは、単一プロセスで常時見る人もいない筆者の運用では読み手が存在しません。残る「因果の正確さ」だけが本物の欠落で、これは `run_id` の実装によって runtime 側へ最小限だけ取り戻しています。
