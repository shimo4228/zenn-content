...
:::details ハマりポイント: ターゲット省略は「呼び出し元」ではなくフォーカス中のペイン
Claude Code に `herdr pane split` を実行させたとき、意図したワークスペースではなく、**人間がそのとき見ていた別のワークスペース**にペインが割れました。ペインの指定を省略すると、「コマンドの呼び出し元ペイン」ではなく「TUI でフォーカス中のペイン」に解決されるためです（公式 CLI リファレンスに明記。`--current` を付ければ呼び出し元ペインに解決されます）。

エージェントに操作させるときは、ターゲットを省略させず、`--current` か `HERDR_PANE_ID` で自分の位置を明示指定させてください。
:::
