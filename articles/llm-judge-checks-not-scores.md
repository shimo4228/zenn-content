---
title: "LLM-as-judge はスコアを集計しない — チェックは証拠、判定は総合判断"
emoji: "⚖️"
type: "tech"
...
**チェックは Yes/No の二値で証拠を集め、判定は名前付きのラベルから 1 つ選ぶ総合判断で出させ、スコア集計はしない。**
...
- 用語: **判定ラベル** = 評価の最終出力。「Keep / Retire」のような名前付きの選択肢から 1 つ選ばせる形にします。英語の文献ではこれを verdict（バーディクト）と呼び、本記事の JSON 例のフィールド名にも `verdict` を使っています
- 用語: **総合判断** = 項目ごとの採点を経由せず、全体を見て 1 つの結論を出す判断。英語の文献では holistic（ホリスティック）評価と呼ばれます
...
flowchart LR
    ...
    B --> C["No 回答 = 証拠"]
    C --> D["② 総合判断<br>名前付きラベルを選ぶ"]
    D --> E["③ 集計しない<br>スコア化・平均化は禁止"]
    E --> F["後段の code / 人間が<br>判定ラベルで処理を分岐"]
```

| 原則 | やること | やらないこと |
|---|---|---|
| ① チェックは二値 | 評価項目を Yes/No で答えられる質問に分解する | 「具体性を 5 段階で採点」 |
| ② 判定は名前付きラベル | 全体を見て `Keep / Improve / Retire` 等から 1 つ選ばせる | 「合計 12 点なので合格」 |
| ③ 集計禁止 | No 回答を判定の**根拠として列挙**させる | Yes の割合（充足率）を品質指標にする |
...
### ② 判定は名前付きラベルを総合判断で選ばせる

総合判断は LLM の得意分野です。前回記事で書いたとおり、LLM は次元ごとの独立採点が苦手で、全体の印象に引きずられます。それなら最初から「全体の印象で 1 つ選べ」と設計する方が素直です。

ポイントは判定ラベルを**次のアクションと 1:1 対応する名前**にすることです。

| 運用例 | 判定ラベルの選択肢 | 対応するアクション |
|---|---|---|
...
`accept / reject` の 2 値より、後段の処理がそのまま分岐できる**固定の選択肢**にする方が実用的です（プログラミングで言う enum・列挙型 — 決められた値以外を取らせない仕組みです。自由記述だと「Improve」「少し改善」のように揺れて、code が分岐できません）。`Merge into [X]` のように**対象の名前を強制する**設計にすると、「どこかと重複している気がする」という曖昧な判定が構造的に書けなくなります。
...
1. No 回答を判定の**根拠として必ず列挙**させる（隠れた No は判定のブレを生みます）
...
> （評価は総合判断であって数値ルーブリックではない。二値回答は判定に食わせる証拠であり、スコアに集計してはならない。充足率は何の判断も変えないうえ、単一の支配的な No を薄める）

## 反証プレッシャーテスト — 判定を確定する前に叩く

二値チェックと総合判断だけでも動きますが、もう 1 段の品質装置を挟むと判定の信頼度が上がります。**仮判定を確定する前に、その判定を反証しようとする質問を LLM 自身に生成・回答させる**ステップです。

```markdown
手順（非 Keep 候補にのみ適用）:
1. 仮判定に対して、それを反証する Yes/No 質問を 1〜3 個生成する
   例: 仮判定が「Update（参照している CLI フラグが廃止済み）」なら
   →「そのフラグは現行バージョンの --help にまだ存在するのではないか？ 実測で確認」
2. 各質問に 1 行の証拠（ファイル読取・パス確認・Web 検索の結果）を付けて回答する
3. 反証が成立した（例: フラグは現存していた）→ 判定を Keep 側に戻す
...
- **一問一主張（atomicity）**: 1 つの質問は 1 つの検証可能な主張だけをテストする
- **反証志向（refutation-oriented）**: 仮判定を肯定する方向ではなく、反証を探す方向に問いを立てる
...
他が全部 Yes でも {非合格側の判定ラベル} を選んでください。
...
`verdict` フィールドは固定の選択肢しか取らないので、受け取った code は `switch` 文で分岐するだけです。`evidence` の No 項目は、そのまま改善タスクのリストとして次の工程（人間または別の LLM）に渡せます。
...
|---|---|---|
| 評価対象 | セッションから抽出した下書き 1 件 | ライブラリ全体（73 スキル） |
...
ところがこの設計で回した監査が**全件 Keep** という結果を返しました。1 件の欠陥も、1 組の重複も検出しない。これは「ライブラリが健全」なのか「評価が空回りしている」のか。切り分けるため、同じライブラリを **10 件前後の小バッチに分けて独立エージェントに精読させる方式**と、**重複検出だけを専任にする方式**で再監査しました（実験の設計と数字の注記は節末の折りたたみにまとめました）。

### 結果 — 落ちる次元と、落ちない次元があった

| 検証項目 | 結果 |
|---|---|
| per-item の欠陥検出（小バッチ精読） | **73 件中 12 件が非 Keep**（16%）。単一コンテキストは全て見逃していた |
| 重複検出（専任プローブ） | 真の重複は **0 件**（17 候補クラスタすべて、役割分担が文書化された隣接スキル） |

見逃された 12 件のうち半数は、404 リンク・削除済みファイルへの参照・廃止された CLI フラグなど、**機械的に検出できる欠陥**でした。極めつけは、**ディスク上に存在しないファイル 2 件に Keep 判定が付いていた**ことです。

一方で重複ゼロは希釈（コンテキストが埋まって注意が薄まること）ではなく実態でした。「全件を一望できるから重複が見える」という単一コンテキストの根拠は、description の軽量スキャン + 狙い撃ちの精読だけで置き換えられていたことになります。
...
flowchart TB
    ...
    P2["Phase 2: per-item 精査（10 件前後の小バッチ・並列）<br>二値チェック + 反証プレッシャーテスト + 総合判断"]
...
:::details 実験の設計と数字の注記（興味のある人向け）
- **Baseline**: 全件単一コンテキストでの監査結果。台帳上は 84 エントリ全件 Keep でした。ディスク上の実ファイルは 73 件で、差分は台帳のバグ——同じスキルが 2 通りのキーで二重計上され、すでに存在しないファイルのエントリまで残っていました。台帳がこれだけ汚れていたこと自体、この時点では誰も気づいていません。以降の検証は実ファイル 73 件を対象にしています
- **処置 A（小バッチ精読）**: 73 件を 10〜11 件×7 バッチに分割し、独立したサブエージェントが同じ基準（二値チェック → 総合判断）で精読。参照の実在は `ls` 等で必ず検証するよう明示
- **処置 B（重複専任プローブ）**: 1 エージェントが全 73 件の description を読んで候補クラスタを列挙 → 本文を並置して精読判定
- **バイアス統制**: baseline が全件 Keep だったことはエージェントに伏せました
- **交絡について正直な注記**: 処置 A のプロンプトは「参照実在を `ls` で必ず検証せよ」を明示していたため、baseline との差の一部は「コンテキスト幅」ではなく「検証の強制の有無」に帰属します。ただし、description と本文の不整合、抽象度の高すぎる記述、スコープ逸脱といった**意味的判断による検出**は検証強制では説明できず、バッチあたり 10 件と 73 件という注意の濃さの差が効いたと解釈しています
:::

## なぜスコアを使わないのか — 提案論文が自ら引いた境界線

「評価基準を二値質問に分解する」アプローチには研究の系譜があります。CheckEval（arXiv:2403.18771、2024）、TICK（arXiv:2410.03608、2024）、そして後発の BinEval「Ask, Don't Judge」（arXiv:2606.27226、2026）です。私の運用は「二値チェック + 総合判断」の骨格が先にあり、この系譜は後から知って各スキルの References に編入しました。BinEval は一問一主張の Yes/No 質問への分解と、失敗した質問を改善フィードバックに直結させる設計を提案しており、反証質問の動的生成と「No 回答 = 改善リスト」の直結は、BinEval の公開後にここから移植した部分です。

面白いのは BinEval 自身の limitations（限界の申告）です。論文は 2 つの留保を自己申告しています。1 つ目: 二値分解が信頼できるのは「コード例があるか」のような**具体的に確認できる基準まで**。「文章として良いか」のような主観的な品質は、人間なら個別チェックの積み上げではなく全体を見て判断するもので、**Yes/No 質問の束に分解しきれない**（less reliable）。2 つ目: 「**充足された質問の割合が品質におおよそ線形対応する**という前提を置いているが、**これは常に成り立つとは限らない**」。つまり「分解した質問の充足率をスコアとして使う」ことには、提案論文自身が留保を付けています。

この限界の申告は、裏返せばそのまま**分業の境界線**です。具体的に確認できる基準は Yes/No に分解できる。だからチェックに証拠集めを任せます。一方、最終的な価値判断は「文章として良いか」型の主観的な層に属し、質問の束にも充足率にも還元しきれない。だからこそ、この層は総合判断として残します。本記事の 3 原則は、二値分解の研究が自己申告した境界線を、そのまま設計に写し取ったものです。

## まとめ

- LLM-as-judge の評価は「二値チェック = 証拠」「判定 = 名前付きラベルから総合判断で 1 つ選ぶ」「集計禁止」の 3 原則で設計します
- 判定ラベルは次のアクションと 1:1 対応する固定の選択肢にし、判定の根拠には No 回答を必ず列挙させます
...
- 「充足率」をスコアにしない根拠は、二値分解の提案論文（BinEval）自身が申告した限界にあります。分解できるのは具体的な基準まで、価値判断の層は総合判断のまま残します
