...
この「LLM が名前付きの選択肢から選び、実行は code が担う」構造は、私が公開している [Agent Knowledge Cycle](https://github.com/shimo4228/agent-knowledge-cycle)（AI エージェントの知識管理フレームワーク。以下 AKC）で **LLM judge + Code enforce** パターンとして定式化しています。判定と実行を分けることで、LLM の判断ミスが直接システム状態を壊すことを防ぎます。
...
スコアを避けるもう 1 つの理由は、運用中に起きた実話です。AKC には評価設計の原則として「Evaluation scales with model capability（評価はモデルの能力に応じてスケールさせる。固定の数値基準はモデルが賢くなっても更新されずに残る）」があります。
