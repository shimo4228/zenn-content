...
この限界の申告は、裏返せばそのまま**分業の境界線**です。具体的に確認できる基準は Yes/No に分解できる。だからチェックに証拠集めを任せます。一方、最終的な価値判断は「文章として良いか」型の主観的な層に属し、質問の束にも充足率にも還元しきれない。だからこそ、この層はホリスティックな verdict として残します。本記事の 3 原則は、二値分解の研究が自己申告した境界線を、そのまま設計に写し取ったものです。
