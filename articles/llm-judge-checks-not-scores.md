...
**チェックは Yes/No の二値で証拠を集め、判定は名前付きのラベルから 1 つ選ぶホリスティック判断で出させ、スコア集計はしない。**
...
> （評価はホリスティック判断であって数値ルーブリックではない。二値回答は判定に食わせる証拠であり、スコアに集計してはならない。充足率は何の判断も変えないうえ、単一の支配的な No を薄める）

## 反証プレッシャーテスト — 判定を確定する前に叩く

二値チェックとホリスティック判定だけでも動きますが、もう 1 段の品質装置を挟むと判定の信頼度が上がります。**仮判定を確定する前に、その判定を反証しようとする質問を LLM 自身に生成・回答させる**ステップです。
...
- **処置 A**: 実スキル 73 件を 10〜11 件×7 バッチ（73 件 ÷ 7）に分割し、独立したサブエージェントが同じ基準（二値チェック → ホリスティック判定）で精読。参照の実在は `ls` 等で必ず検証するよう明示
...
flowchart TB
    ...
    P2["Phase 2: per-item 精査（10 件前後の小バッチ・並列）<br>二値チェック + 反証プレッシャーテスト + ホリスティック判定"]
...
この限界の申告は、裏返せばそのまま**分業の境界線**です。具体的に確認できる基準は Yes/No に分解できる。だからチェックに証拠集めを任せます。一方、最終的な価値判断は「文章として良いか」型の主観的な層に属し、質問の束にも充足率にも還元しきれない。だからこそ、この層はホリスティックな判定として残します。本記事の 3 原則は、二値分解の研究が自己申告した境界線を、そのまま設計に写し取ったものです。

## まとめ

- LLM-as-judge の評価は「二値チェック = 証拠」「判定 = 名前付きラベルからホリスティックに 1 つ選ぶ」「集計禁止」の 3 原則で設計します
...
- 「充足率」をスコアにしない根拠は、二値分解の提案論文（BinEval）自身が申告した限界にあります。分解できるのは具体的な基準まで、価値判断の層はホリスティックに残します
