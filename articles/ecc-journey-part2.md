...
この記事では、pdf2anki を作る過程で踏んだ **6 つの落とし穴** と、その防御策を書きます。Claude API をこれから使う人が同じ穴を避けられるように。
...
**「成功か失敗の二択」ではなく「部分的な成功を許容する」設計。** これは LLM を組み込むツール全般に通用する原則です。
...
全カードを LLM に再評価させる方式を最初に試しました。生成と評価の両方で API を叩くので、コストが単純に倍になります。100 枚のカードを作るたびに 200 回の API コール。現実的ではありません。
...
「また、」「さらに」が出てきたら、1 枚のカードに情報を詰め込みすぎている可能性が高いです。人間が見ても分割すべきと判断できるカードを、コードで検出できます。
...
`frozen=True` で不変にしています。前回の記事で触れた「不変性の原則」がここで効きました。API コールのたびに新しい `CostTracker` インスタンスを返します。途中で値が書き換わることはありません。「いまいくら使ったか」を常に正確に追跡できます。
...
1 つ目。Markdown 見出しがないテキストへの対応です。日本語の書籍なら「第1章」「（1）」「一、」といったパターンで見出しを検出するフォールバックを入れました。
...
プロンプトを書き換えたとき、カードの品質への影響を判定できません。「良くなった気がする」では心許ないです。LLM の出力は非決定的で、同じプロンプトでも毎回違う結果が出ます。
...
キーワードマッチには限界があります。「ニューラルネットワーク」と「神経回路網」を同一視できません。
...
開発全体を通して効いた判断基準が1つあります。**「これをやらない場合、同じ時間で何ができるか」** を常に問うことです。

途中で「OpenAI にも対応すべきでは」と考えました。$10 クレジットが残っていたからです。実装を見積もると 1,000 行、10 時間。$10 の回収に時給 $1 で働く計算です。同じ 10 時間で画像カード生成、TUI、Eval フレームワークが作れました。
