...
その日のうちに元論文のコードを clone し、計画から実装完了まで約 2 時間で PyTorch を全部剥がして純 NumPy で再実装し、Streamlit でブラウザから動かせるようにしていた。1550 行。テストカバレッジ 98%。
...
**計算論的な神経科学の論文を、理論を理解していない人間が再実装できてしまった。** PyTorch のコードを読み、NumPy に書き換え、Jacobian を解析的に導出し、テストまで書いた。所要時間は約 2 時間。
...
能動的推論のツールを作れる人間は、Claude Code があれば無数に生まれる。でも「毎朝のリサーチレポートの片隅にあった一単語へ引っかかって、理論も分からないのに再実装し始める人間」は、たぶんそう多くない。実装力が民主化された世界で残る差別化要因は、**何を作るかを決める偏愛**だけだ。
...
再実装の対象は Priorelli et al. (2025) "[Embodied decisions as active inference](https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1012745)"。PLOS Computational Biology に掲載された論文で、能動的推論を使って「身体化された意思決定」をモデル化している。
...
元コードは [PyTorch + Pymunk + Pyglet](https://github.com/priorelli/embodied-decisions) で書かれている。GPU 環境が必要で、ブラウザからは動かせない。

## 設計判断: なぜ全部書き直したのか——と Claude Code は言う
...
再実装の 8 割は淡々と進んだ。Claude Code にドキュメントとして論文記法→コード変数のマッピング表と元コードの構造マップを渡し、ファイルごとに書き換えていった。私は隣で見ているだけだ。「ふーん、そういう構造なんだ」と思いながら。
...
250MB の PyTorch + Pymunk + Pyglet が、50MB の NumPy + SciPy + Streamlit に置き換わった。この判断が理論的に妥当なのかは分からない。ただ、ブラウザでパラメータを変えながらリアルタイムに何かが動いているのは見える。「信念」と名付けられた値が変化し、アームが目標に向かって動く。それが能動的推論として正しい挙動なのかは、私には判断できない。

## プロジェクト構成

Claude Code が 2 時間で書いた 1550 行の構成を載せておく。
...
Claude Code は恐ろしいツールだ。計算論的な神経科学の論文再実装を、素人が 2 時間でやれてしまう。テストカバレッジ 98%、型チェック完全対応。プロの研究者なら致命的な誤りを指摘するだろうか。それとも「形式的にはよくできている」と言うだろうか。自分にはどちらか判断する力がない。
