...
ただし**メンタルモデルであってテンプレートではない**。立場表明（マニフェスト）記事にはこのフレーム自体を被せない — 運用は zenn-editorial-judgment の「記事タイプの事前判定」。書き方は zenn-practical-writing「導入の設計」。
...
系として、著者の実発言（**規則そのものはここに持たない** — 運用も検出手順も
`zenn-editorial-judgment` が正本: 著者の事実の確認 :83、数値の実測主義 :87、クレーム校正 :130）:

> 「もっともらしいけど、私はこんなハーネス組んでない」（2026-07-26 — 著者がしていないことを書かない）

> 「2 時間は根拠があるのか？Claude の時間見積もりはいつも何倍も長い」（2026-07-27 — 数値は実測のみ）

> Chaos Engineering → fault injection testing へのクレーム軌道修正（2026-07-13 — 手法名は正確に）
