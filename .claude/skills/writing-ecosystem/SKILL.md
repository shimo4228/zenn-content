...
First-screen deliverable: <実用 channel のみ。最初の見出しの前で読者がそのまま使える 1 個（コマンド / プロンプト / repo / 流れ 1 行 / hero 図）。hero 図なら「何を描くか」だけをここに書き、起こすのは内容 GO 後。埋まらなければ essay channel を疑う>
...
図は内容 GO の後に起こす（brief の Figure plan をここで埋める。手順は下の「認知負荷の設計」）。
...
実用 channel（Zenn / Dev.to）の既定。読者は検索・feed から来て数秒で用途を判断する。

- **第一画面は成果物。** 最初の見出しの前に、読者がそのまま使える 1 個を置く（コマンド / プロンプト /
  repo link / 流れ 1 行 / hero 図）。「この記事では〜」の予告文は成果物の代わりにならない。brief の
  `First-screen deliverable` が埋まらない稿は、Zenn でなく note 向きの信号として著者に返す
- **図は凍結後に、節ごとに。** 内容 GO の後、節ごとに形を 1 語で言う（対比 / 流れ / 階層 / 2 軸 / 並列）。
  ...
  出そろった段落の後（先に図を出すと節の展開を先取りする）。例外は hero 図で、最初の見出しの前に置く。
...
- **証拠は埋め込みへ。** 実験ログ・設定全文・ADR は link card か `:::details` に逃がし、本文は判断・数値・
  trade-off だけを残す
- **長さの参照値。** Zenn 本文は 6,300 字（2026-09 の Zenn いいね上位 30 本の中央値。著者稿は 9,500 字）。
  上限ではなく、超えたら「図 1 枚か埋め込みで置き換えられる節は無いか」を問う契機にする

review-when: pilot（`articles/jev-retrofit-limits.md`、2026-09-22）を含む 3 本の `article-stocktake` で
